...
import { getResourceAllocations, createResourceAllocation, ResourceAllocationWithProfile } from "@/services/resourceService";
import { getAllProfiles } from "@/services/profileService";
import { Plus, Loader2, Users, Check, UserCircle2 } from "lucide-react";
        ...
        <form onSubmit={handleCreateAllocation} className="bg-slate-50 p-6 rounded-2xl border border-slate-200 mb-8 fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div>
              ...
              <select required value={selectedUserId} onChange={(e) => setSelectedUserId(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2 text-sm outline-none focus:border-[var(--color-brand-orange)]">
            ...
            <div>
              ...
              <select value={functionalRole} onChange={(e) => setFunctionalRole(e.target.value as any)} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2 text-sm outline-none focus:border-[var(--color-brand-orange)]">
      ...
      {allocations.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
           {allocations.map(alloc => (
             <div key={alloc.id} className="bg-white border border-slate-200 p-5 rounded-[1.5rem] shadow-sm hover:shadow-md transition">
                <div className="flex items-center gap-3 mb-4">
                   <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-400">
                     <UserCircle2 size={24} />
                   </div>
                   <div>
                     <h4 className="font-bold text-slate-800 leading-tight">{alloc.profile?.full_name || "Unknown"}</h4>
